...
class LeaderboardQueryTest < ActiveSupport::TestCase
  def setup
    ...
    3.times { |i| create_catch(user: @ann, variant: @variant, species: @bass, length_cm: 30 + i * 10, upvotes_count: i == 0 ? 10 : 0) }
    # Bob: 1 bass + 1 pike, 5 upvotes, best length 80cm
    create_catch(user: @bob, variant: @variant, species: @bass, length_cm: 80, upvotes_count: 5)
    create_catch(user: @bob, variant: @variant, species: @pike, length_cm: 40)
  end

  test "ranks by length (personal best) by default" do
    rows = LeaderboardQuery.new.rows
    assert_equal @bob, rows.first.user
    assert_equal 80, rows.first.best_length_cm
  end

  test "ranks by catches" do
    rows = LeaderboardQuery.new(metric: :catches).rows
