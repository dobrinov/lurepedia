...
# into the user's preferred unit system.
class Units
  CM_PER_IN = 2.54
  CM_PER_FT = 30.48
  G_PER_OZ = 28.3495
  ...
  # Resolve :auto against the active (or given) locale.
  def self.system(setting, locale: I18n.locale)
    setting = setting.to_s
    return setting.to_sym if %w[imperial metric].include?(setting)

    imperial_locale?(locale) ? :imperial : :metric
  end

  def self.imperial_locale?(locale)
    %i[en].include?(locale.to_sym)
  end

  def self.format_length(cm, setting, locale: I18n.locale)
    return nil if cm.blank?

    cm = cm.to_f
    if system(setting, locale: locale) == :imperial
  ...
  def self.format_weight(grams, setting, locale: I18n.locale)
  ...
  def self.format_depth(min_cm, max_cm, setting, locale: I18n.locale)
    return nil if min_cm.blank? && max_cm.blank?

    imperial = system(setting, locale: locale) == :imperial
